...
public class Base64 {
	...
	/*
	 * Host platform me be something funny like EBCDIC, so we hard code these
	 * values.
	 */
	...
	private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
		...
		// Example: DkLE
			...
			try {
				...
				// "Problem writing object", e );
				// logger.error( Logger.SECURITY_FAILURE, ""+source[srcOffset]+
				// ": " + ( DECODABET[ source[ srcOffset ] ] ) );
				// logger.error( Logger.SECURITY_FAILURE,
				// ""+source[srcOffset+1]+ ": " + ( DECODABET[ source[ srcOffset
				// + 1 ] ] ) );
				// logger.error( Logger.SECURITY_FAILURE,
				// ""+source[srcOffset+2]+ ": " + ( DECODABET[ source[ srcOffset
				// + 2 ] ] ) );
				// logger.error( Logger.SECURITY_FAILURE,
				// ""+source[srcOffset+3]+ ": " + ( DECODABET[ source[ srcOffset
				// + 3 ] ] ) );

				// CHECKME: I replaced the 5 separate logger.error() calls above
				// with a single logger.error() call so they can't
				// become interleaved with other log entries from other threads.
				// Normally this would have placed log entries
				// on separate lines, so I also added line terminators here as
				// well. (Probably don't want it all on one single
				// really long log entry, do we?) Anyhow, somebody should check
				// the formatting to ensure that it's
				// esthetically pleasing, etc. But this works for me. I'm also
				// OK if you want to remove all the line terminators
				// in which case the declaration for EOL should be removed as
				// well. - Kevin Wall
	...
	public static byte[] decode(String s, int options) {
		...
		try {
			...
			// CHECKME: Is this correct? I think it should be a warning instead
			// of an error since nothing
			// is re-thrown. I do think that *some* sort of logging is in order
			// here especially since UTF-8 should
			// always be available on all platforms. If it's not, then all bets
			// are off on your runtime env. - Kevin Wall
